...
export async function installDependceny(config?: InstallDependcenyConfig): Promise<boolean> {
    ...
    await spawnAsync(
        `${manager} install${global.__ZIXULU_REGISTRY__ ? ` --registry ${global.__ZIXULU_REGISTRY__}` : ""}${global.__ZIXULU_PROXY__ ? ` --proxy http://localhost:7890 --https-proxy http://localhost:7890` : ""}`,
