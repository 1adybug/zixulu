...
export async function installDocker() {
    ...
    if (distributorId !== "Ubuntu")
        throw new Error("暂不支持 Ubuntu 以外的系统")
    ...
    const hrefs = Array.from(dom.window.document.querySelectorAll("a")).map(
        ({ href }) => href,
    )
    ...
    const deb4 = hrefs.findLast(item =>
        item.startsWith("docker-buildx-plugin_"))!
    const deb5 = hrefs.findLast(item =>
        item.startsWith("docker-compose-plugin_"))!
    ...
    await Promise.all(
        debs
            ...
            .map(deb =>
                execAsync(
                    `wget -q https://download.docker.com/linux/ubuntu/dists/${codename}/pool/stable/amd64/${deb}`,
                )),
    )
