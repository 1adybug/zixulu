...
import { regexPlaceholderHint } from "@/utils/preprocessRegex"
...
program
    ...
    .action(async (infos: string[]) => {
        ...
        if (infos.length === 0) throw new Error("请输入作者信息！")
            ...
            if (infos.length === 1) next = getUsernameAndEmail(infos[0])
            else {
...
program
    .command("replace-tag")
    .alias("rt")
    .description(`替换所有 git tag (${regexPlaceholderHint})`)
...
program
    .command("remove-tag")
    .alias("rmt")
    .description(`删除匹配的 git tag (${regexPlaceholderHint})`)
    .argument("regexp", "正则表达式字符串")
    .option("-f, --flags <flags>", "正则表达式标志，如 i、g、m 等")
...
program
    .command("add-tag")
    .alias("adt")
    .description(`为匹配的 commit 添加 git tag (${regexPlaceholderHint})`)
...
program
    ...
    .alias("rcm")
    .description(`替换所有提交消息（⚠️ 会重写 Git 历史，${regexPlaceholderHint}）`)
    .argument("regexp", "正则表达式字符串")
    .argument("[replace]", "替换字符串，默认为空字符串")
